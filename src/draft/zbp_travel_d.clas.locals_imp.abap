...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler
 FRIENDS ltc_travel.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      t_entities_create TYPE TABLE FOR CREATE zr_travel_d\\travel,
      t_entities_update TYPE TABLE FOR UPDATE zr_travel_d\\travel,
      t_failed_travel   TYPE TABLE FOR FAILED   EARLY zr_travel_d\\travel,
      t_reported_travel TYPE TABLE FOR REPORTED EARLY zr_travel_d\\travel.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZR_Travel_D.
    ...
    LOOP AT keys_with_valid_discount ASSIGNING FIELD-SYMBOL(<key_with_valid_discount>) WHERE %param-discount_percent IS INITIAL
      ...
      APPEND VALUE #( %tky                       = <key_with_valid_discount>-%tky
                      %msg                       = NEW zcm_flight_messages(
                                                       textid = zcm_flight_messages=>discount_invalid
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS  INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        %state_area         = 'VALIDATE_CUSTOMER'
                        %msg                = NEW zcm_flight_messages(
                                                                textid   = zcm_flight_messages=>enter_customer_id
                                                                severity = if_abap_behv_message=>severity-error )
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        %state_area         = 'VALIDATE_AGENCY'
                        %msg                = NEW zcm_flight_messages(
                                                          textid   = zcm_flight_messages=>enter_agency_id
                                                          severity = if_abap_behv_message=>severity-error )
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-BeginDate IS INITIAL.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        %state_area        = 'VALIDATE_DATES'
                         %msg              = NEW zcm_flight_messages(
                                                                textid   = zcm_flight_messages=>enter_begin_date
                                                                severity = if_abap_behv_message=>severity-error )
      ...
      IF travel-EndDate IS INITIAL.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        %state_area        = 'VALIDATE_DATES'
                         %msg                = NEW zcm_flight_messages(
                                                                textid   = zcm_flight_messages=>enter_end_date
                                                                severity = if_abap_behv_message=>severity-error )
  ...
  METHOD get_global_authorizations.
    ...
    IF requested_authorizations-%create EQ if_abap_behv=>mk-on.
      ...
      ELSE.
        ...
        APPEND VALUE #( %msg    = NEW zcm_flight_messages(
                                       textid   = zcm_flight_messages=>not_authorized
                                       severity = if_abap_behv_message=>severity-error )
...
*      ENDCASE.
      ...
      IF sy-subrc = 0.
        ...
        IF update_requested = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky = travel-%tky
                            %msg = NEW zcm_flight_messages(
                                                     textid    = zcm_flight_messages=>not_authorized_for_agencyid
                                                     agency_id = travel-AgencyID
                                                     severity  = if_abap_behv_message=>severity-error )
                            %element-AgencyID = if_abap_behv=>mk-on
                           ) TO reported-travel.
  ...
  METHOD precheck_auth.
    DATA:
      ...
      agencies          TYPE SORTED TABLE OF zagency WITH UNIQUE KEY agency_id,
