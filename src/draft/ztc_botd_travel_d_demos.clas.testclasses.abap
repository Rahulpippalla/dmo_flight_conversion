...
"! <li>The dependent BO is zr_travel_d and the code under test is its consumer {@link ztc_travel_d_bo_consumer}</li>
...
"! </ul>
"! For examples with isolating EML dependencies on active instances, refer to the class {@link ztc_botd_travel_m_demos}.
...
class ltcl_using_mockemlapi_variant definition final for testing
  ...
  private section.
    ...
    class-data cut type ref to ztc_travel_d_bo_consumer.
...
class ltcl_using_mockemlapi_variant implementation.
  ...
  method class_setup.
  "Create doubles for BO 'ZR_TRAVEL_D'.
  "a. Prepare environment configuration with bdef dependencies for which doubles are to be created
  data(env_config) = cl_botd_mockemlapi_bo_test_env=>prepare_environment_config(  )->set_bdef_dependencies( bdef_dependencies = value #( ( 'ZR_TRAVEL_D' ) )
                                                                                  )->handle_draft( bdef_dependencies = value #( ( 'ZR_TRAVEL_D' ) ) ).
  ...
  "For Create on Travel entity
   data create_travel_instances type table for create ZR_TRAVEL_D.
  ...
  "Step 2: Define and set up the response structures to be returned for Modify EML in CUT
    data mapped type response for mapped early ZR_TRAVEL_D.
    data failed type response for failed early ZR_TRAVEL_D.
  ...
  "Step 3: Create input and output configurations for the Modify EML.
    ...
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'
  ...
  "Step 4: Configure the Modify EML in CUT via configure_call API on the double.
    double =  environment->get_test_double( 'ZR_TRAVEL_D' ).
  ...
  "Step 1: Setup test data instances for all operations on all entities in Modify EML (Here only create_ba ).
   data cba_booking_instances type table for create ZR_TRAVEL_D\_Booking.
  ...
  "Step 3: Create input and output configurations for the Modify EML.
    ...
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'   "can accept the entity name or alias name
  ...
  "For Update on Booking entity
   data update_booking_instances type table for update ZR_TRAVEL_D\\booking.
  ...
  "Step 3: Create input and output configurations for the Modify EML.
    ...
    "Create input for all entity parts
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'   "can accept the entity name
                                   )->set_instances_for_create( create_travel_instances
                                   )->set_instances_for_create_ba( cba_booking_instances ).
  ...
  "*************Configure second EML in Code under test*************
    ...
    data create_travel_instances2 type table for create ZR_TRAVEL_D.
    ...
    data(eml_travel_input2) = input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'   "can accept the entity name
...
*  double->verify( )->modify( input2 )->is_called_times( times = 2 ). "Since input2 is a subset of input. Thus both EMLs are infact executed with input2
  ...
  data(verification_input) = input_config_builder_4_modify->build_input_for_eml(
                                                                     ...
                                                                     input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'
                                                                            )->set_instances_for_create( create_travel_instances2 ) ).
...
"  double->verify( )->modify( verification_input )->is_called_times( times = 2 ).
  ...
  "Step 3: Create input and output configurations for the Modify EML.
   ...
   data(output_config_builder_4_modify) = cl_botd_mockemlapi_bldrfactory=>get_output_config_builder( )->for_modify( ).
    ...
    "Create input for all entity parts and set instances for all operations on that entity
    "For travel entity
   data(travel_part) = input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'   "can accept the entity name
                                   )->set_instances_for_create( create_travel_instances ).
  ...
  "For Update on Travel entity
   data update_travel_instances type table for update ZR_TRAVEL_D.
  ...
  "For accept travel action on Travel entity
   data accept_travel_act_insts type table for action import ZR_TRAVEL_D~acceptTravel.
  ...
  "For deduct discount action on Travel entity
   data deduct_discount_act_insts type table for action import ZR_TRAVEL_D~deductDiscount.
   deduct_discount_act_insts = value #( ( TravelUUID = '987' %param-discount_percent = 10 %is_draft = if_abap_behv=>mk-on ) )."Should pass

  "Step 2: Define and set up the response structures to be returned for Modify EML in CUT
    data mapped_accept_travel type response for mapped early ZR_TRAVEL_D.
    data mapped_deductDiscount type response for mapped early ZR_TRAVEL_D.
    data failed type response for failed early ZR_TRAVEL_D.
    data result_accept_travel type table for action result ZR_TRAVEL_D~acceptTravel.
    data result_deductDiscount type table for action result ZR_TRAVEL_D~deductDiscount.
  ...
  "Step 4: Configure the Modify EML in CUT via configure_call API on the double for Action accept Travel.
    ...
    data(eml_travel_input_deduct_discnt) = input_config_builder_4_modify->build_entity_part( 'ZR_TRAVEL_D'
  ...
  "For Creating booking_supplement by association on Booking entity
   data cba_booking_suppl_instances type table for create ZR_TRAVEL_D\\booking\_BookingSupplement.
  ...
  "For delete on Travel entity
   data delete_travel_instances type table for delete ZR_TRAVEL_D.
  ...
  "For read on travel
   data read_travel_instances type table for read import ZR_TRAVEL_D.
  ...
  "Step 2: Define and set up the response structures to be returned for Read EML in CUT
    data result type table for read result ZR_TRAVEL_D.
  ...
  "Step 3: Create input and output configurations for Read EML.
   ...
   data(output_config_builder_4_read) = cl_botd_mockemlapi_bldrfactory=>get_output_config_builder( )->for_read( ).
    ...
    "Create input for all entity parts
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_read->build_entity_part( 'ZR_TRAVEL_D'   "can accept the entity name
  ...
  "For read booking by association on travel
   data rba_booking_instances type table for read import ZR_TRAVEL_D\_Booking.
  ...
  "Step 2: Define and set up the response structures to be returned for Read EML in CUT
    data result type table for read result ZR_TRAVEL_D\_Booking.
...
class ltcl_using_txbufdbl_variant implementation.
  ...
  "Create doubles for BO 'ZR_TRAVEL_D'.
  "a. Prepare environment configuration with bdef dependencies for which doubles are to be created
    data(env_config) = cl_botd_txbufdbl_bo_test_env=>prepare_environment_config( )->set_bdef_dependencies( bdef_dependencies = value #( ( 'ZR_TRAVEL_D' ) )
                                                                                  )->handle_draft( bdef_dependencies = value #( ( 'ZR_TRAVEL_D' ) ) ).
  ...
  "Set numbering support on double (if required).
    data(double) =  environment->get_test_double( 'zr_travel_d' ).
    ...
    modify entities of zr_travel_d
     entity travel
  ...
  "Step 3: Define and set up the mapped response structure to be returned for Create EML on test data instance in CUT
    data mapped_987 type response for mapped early zr_travel_d.
    data mapped_988 type response for mapped early zr_travel_d.
  ...
  """""""""""""""""""""""""""CODE UNDER TEST"""""""""""""""""""""""""""""""""""""""
  "For Create on Travel entity
   data create_travel_instances_cut type table for create zr_travel_d.
...
*   data travel_instances type table for create zr_travel_d.
...
*    data(double) = environment->get_test_double( root_name = 'zr_travel_d' ).
...
*
  ...
  "Step 1: Insert test data in transactional buffer double using a CREATE EML (if required).
   ...
   data mapped_travel_987 type response for mapped early zr_travel_d.
  ...
  "Step 3: Define and set up the mapped response structure to be returned for Create by association EML on test data instance in CUT, with the key to be assigned for the instance.
    data mapped_booking_001 type response for mapped early zr_travel_d.
  ...
  "Step 3: Define and set up the mapped response structure to be returned for update EML on test data instance in CUT.
    data mapped_update type response for mapped early zr_travel_d.
