...
CLASS lhc_booking IMPLEMENTATION.
  ...
  METHOD setBookingNumber.
    DATA max_bookingid TYPE zbooking_id.
    DATA bookings_update TYPE TABLE FOR UPDATE ZR_Travel_D\\Booking.
    ...
    LOOP AT bookings ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      IF <booking>-AirlineID IS INITIAL.
        ...
        APPEND VALUE #( %tky                = <booking>-%tky
                        %state_area         = 'VALIDATE_CONNECTION'
                         %msg                = NEW zcm_flight_messages(
                                                                textid = zcm_flight_messages=>enter_airline_id
                                                                severity = if_abap_behv_message=>severity-error )
                        %path              = VALUE #( travel-%tky = travel_booking_links[ KEY id  source-%tky = <booking>-%tky ]-target-%tky )
      ...
      IF <booking>-ConnectionID IS INITIAL.
        ...
        APPEND VALUE #( %tky                = <booking>-%tky
                        %state_area         = 'VALIDATE_CONNECTION'
                        %msg                = NEW zcm_flight_messages(
                                                                textid = zcm_flight_messages=>enter_connection_id
                                                                severity = if_abap_behv_message=>severity-error )
                        %path               = VALUE #( travel-%tky = travel_booking_links[ KEY id  source-%tky = <booking>-%tky ]-target-%tky )
      ...
      IF <booking>-FlightDate IS INITIAL.
        ...
        APPEND VALUE #( %tky                = <booking>-%tky
                        %state_area         = 'VALIDATE_CONNECTION'
                        %msg                = NEW zcm_flight_messages(
                                                                textid = zcm_flight_messages=>enter_flight_date
                                                                severity = if_abap_behv_message=>severity-error )
                        %path               = VALUE #( travel-%tky = travel_booking_links[ KEY id  source-%tky = <booking>-%tky ]-target-%tky )
      ...
      IF <booking>-AirlineID IS NOT INITIAL AND
        ...
        SELECT SINGLE Carrier_ID, Connection_ID, Flight_Date   FROM zflight  WHERE  carrier_id    = @<booking>-AirlineID
        ...
        IF sy-subrc <> 0.
          ...
          APPEND VALUE #( %tky                 = <booking>-%tky
                          %state_area          = 'VALIDATE_CONNECTION'
                          %msg                 = NEW zcm_flight_messages(
                                                                textid      = zcm_flight_messages=>no_flight_exists
