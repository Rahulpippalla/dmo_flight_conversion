...
CLASS lhc_carrier IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    DATA: airline_ids TYPE SORTED TABLE OF zi_carrier WITH UNIQUE KEY AirlineID.

    READ ENTITIES OF ZI_CarriersLockSingleton_S IN LOCAL MODE
      ENTITY Carrier
        FIELDS ( AirlineID )
    ...
    LOOP AT Carrier ASSIGNING FIELD-SYMBOL(<carrier>).
      ...
      IF line_exists( connections_db[ AirlineID = <carrier>-AirlineID ] ).
        ...
        APPEND VALUE #( %tky                        = <carrier>-%tky
                        %msg                        = NEW zcx_carriers_s(
                                                       textid        = zcx_carriers_s=>airline_still_used
  ...
  METHOD validateCurrencyCode.
    ...
    LOOP AT Carrier ASSIGNING FIELD-SYMBOL(<carrier>).
      ...
      IF <carrier>-CurrencyCode IS INITIAL.
        ...
        APPEND VALUE #( %tky                             = <carrier>-%tky
                        %msg                             = NEW zcx_carriers_s(
                                                               textid     = zcx_carriers_s=>currency_code_required
      ...
      ELSEIF NOT line_exists( currency_codes_db[ Currency = <carrier>-CurrencyCode ] ).
        ...
        APPEND VALUE #( %tky                             = <carrier>-%tky
                        %msg                             = NEW zcx_carriers_s(
                                                               textid        = zcx_carriers_s=>invalid_currency_code
                                                               severity      = if_abap_behv_message=>severity-error
  ...
  METHOD validateName.
    ...
    LOOP AT Carrier ASSIGNING FIELD-SYMBOL(<carrier>).
      ...
      IF <carrier>-name IS INITIAL.
        ...
        APPEND VALUE #( %tky                        = <carrier>-%tky
                        %msg                        = NEW zcx_carriers_s(
                                                         textid     = zcx_carriers_s=>name_required
                                                         severity   = if_abap_behv_message=>severity-error
                                                         airline_id = <carrier>-AirlineID )
