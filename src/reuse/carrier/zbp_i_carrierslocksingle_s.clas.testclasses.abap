"! @testing BDEF:ZI_CARRIER
CLASS lthc_carrier DEFINITION FINAL FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
  PRIVATE SECTION.
    CONSTANTS airlineid_for_test TYPE zcarrier_id VALUE 'XX4' ##NO_TEXT.
    ...
    DATA:
      failed        TYPE RESPONSE FOR FAILED   EARLY ZI_CarriersLockSingleton_S,
      reported      TYPE RESPONSE FOR REPORTED EARLY ZI_CarriersLockSingleton_S,
      failed_late   TYPE RESPONSE FOR FAILED   LATE  ZI_CarriersLockSingleton_S,
      reported_late TYPE RESPONSE FOR REPORTED LATE  ZI_CarriersLockSingleton_S.
...
CLASS lthc_carrier IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities =  VALUE #(
            ( i_for_entity = 'ZI_CARRIER_S'  )
            ( i_for_entity = 'ZI_Connection' )
          )
      ).
  ENDMETHOD.
  ...
  METHOD validateCurrencyCode_success.
    DATA:
      carrier  TYPE ZI_Carrier,
      carriers TYPE STANDARD TABLE OF ZI_Carrier.

    carrier = VALUE ZI_Carrier(
  ...
  METHOD get_instance_features.
    DATA:
      result                     TYPE  TABLE FOR INSTANCE FEATURES RESULT zi_carrierslocksingleton_s\\carrier,
      carrier_with_connection    TYPE ZI_Carrier,
      carrier_without_connection TYPE ZI_Carrier,
      carriers                   TYPE STANDARD TABLE OF ZI_Carrier,
      connection                 TYPE zconnection,
      connections                TYPE STANDARD TABLE OF zconnection.

    carrier_with_connection = VALUE ZI_Carrier(
        AirlineID = 'AAA'
      ).

    carrier_without_connection = VALUE ZI_Carrier(
        AirlineID = 'BBB'
      ).
...
CLASS lthc_carrierslocksingleton IMPLEMENTATION.
  ...
  METHOD get_global_authorizations.
    DATA:
      ...
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZI_CarriersLockSingleton_S\\CarriersLockSingleton,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZI_CarriersLockSingleton_S\\CarriersLockSingleton,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY ZI_CarriersLockSingleton_S.
...
CLASS ltsc_I_CARRIERSLOCKSINGLETON_S DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA:
      ...
      mapped       TYPE RESPONSE FOR MAPPED   LATE ZI_CarriersLockSingleton_S,
      failed       TYPE RESPONSE FOR FAILED   LATE ZI_CarriersLockSingleton_S,
      reported     TYPE RESPONSE FOR REPORTED LATE ZI_CarriersLockSingleton_S.
    ...
    METHODS:
      "! Checks if { @link ..lsc_I_CARRIERSLOCKSINGLETON_S.METH:save_modified } calls { @link FUNC:ZFLIGHT_TRAVEL_SAVE }
