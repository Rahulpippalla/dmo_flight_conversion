...
CLASS lcl_abstract DEFINITION ABSTRACT CREATE PUBLIC.
  ...
  PROTECTED SECTION.
    TYPES:
      ts_agency_with_indicators TYPE zagency WITH INDICATORS control.
...
CLASS lcl_slogan DEFINITION
  ...
  PUBLIC SECTION.
    TYPES:
      BEGIN OF t_slogan,
        id          TYPE i,
        text        TYPE zzz_free_text_comment,
...
CLASS lcl_slogan IMPLEMENTATION.
  ...
  METHOD generate_slogan.
    ...
    UPDATE zagency FROM TABLE @gt_agency INDICATORS SET STRUCTURE control.
  ENDMETHOD.
  ...
  METHOD _slogan_iterate.
    ...
    LOOP AT gt_agency ASSIGNING FIELD-SYMBOL(<agency>).
      <agency>-control-zzzsloganzag = if_abap_behv=>mk-on.
      DATA(next) = roots[ floor( go_rnd->get_next( ) * lines( roots ) ) + 1 ].
      <agency>-zzzsloganzag = next-text.
      WHILE next-followed_by IS NOT INITIAL.
        ...
        <agency>-zzzsloganzag &&= ` ` && next-text.
      ENDWHILE.
      REPLACE ALL OCCURRENCES OF agencys_name IN <agency>-zzzsloganzag WITH <agency>-name.
    ENDLOOP.
  ENDMETHOD.
...
CLASS lcl_review_generator DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      tt_review                 TYPE STANDARD TABLE OF zzz_agn_reva WITH KEY agency_id review_id.
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      _get_review_rating_avg
        RETURNING
          VALUE(rv_review_rating_average) TYPE zzz_rating.
...
CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS:
      constructor
        IMPORTING
          is_agency         TYPE zagency
          iv_rating_average TYPE zzz_rating,
      generate
        RETURNING
          VALUE(rs_review) TYPE zzz_agn_reva.

  PROTECTED SECTION.
    DATA:
      mv_rating_average TYPE zzz_rating.

  PRIVATE SECTION.
    TYPES:
      BEGIN OF ts_id_map,
        agency_id TYPE zagency_id,
        review_id TYPE zzz_review_id,
      END OF ts_id_map,
    ...
    METHODS:
      _get_review_rating
        IMPORTING
          iv_target_rating        TYPE zzz_rating
        RETURNING
          VALUE(rv_review_rating) TYPE zzz_rating,
    ...
    DATA:
      ms_agency TYPE zagency,
      ms_review TYPE zzz_agn_reva.
ENDCLASS.
  ...
  PUBLIC SECTION.
    TYPES:
      ...
      BEGIN OF t_review_text_edges,
        id                 TYPE i,
        followed_by_rating TYPE zzz_rating,
        followed_by        TYPE i,
    ...
    METHODS:
      generate_text_for_agency
        IMPORTING
          is_agency      TYPE zagency
        RETURNING
          VALUE(rv_text) TYPE zzz_free_text_comment.
  PROTECTED SECTION.
  PRIVATE SECTION.
    CONSTANTS:
    ...
    DATA:
      mv_rating       TYPE zzz_rating,
      ms_agency       TYPE zagency,
      mv_text         TYPE zzz_free_text_comment,
...
CLASS lcl_review_generator IMPLEMENTATION.
  ...
  METHOD generate_reviews.
    ...
    DELETE FROM zzz_agn_reva.                       "#EC CI_NOWHERE

    INSERT zzz_agn_reva FROM TABLE @mt_reviews_to_create.
