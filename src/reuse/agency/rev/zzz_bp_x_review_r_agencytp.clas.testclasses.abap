CLASS ltcl_review DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CLASS-DATA:
      class_under_test     TYPE REF TO lhc_zzz_review,
    ...
    METHODS:
      "! Calls { @link ..lhc_ZZZ_Review.METH:get_instance_features }
      "! with all possible in- and outcomes.
      get_instance_features        FOR TESTING,

      "! Checks that { @link ..lhc_ZZZ_Review.METH:get_global_authorizations } returns initial values
...
CLASS ltcl_review IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                               VALUE #(
                                   ( i_for_entity = 'ZI_AGENCYTP'            )
                                   ( i_for_entity = 'ZZZ_I_AGENCY_REVIEWTP'  )
                                 )
                             ).
  ENDMETHOD.
  ...
  METHOD get_instance_features.
    TYPES: t_instance_feature TYPE STRUCTURE FOR INSTANCE FEATURES RESULT zr_agencytp\\zzz_review,
           BEGIN OF t_check.
             INCLUDE TYPE t_instance_feature.
    TYPES: reviewer TYPE zzz_r_agency_reviewtp-reviewer,
           END OF t_check,
           t_check_table TYPE STANDARD TABLE OF t_check WITH KEY agencyid reviewid.

    DATA:
      check_table        TYPE t_check_table,
      review_mock_data   TYPE STANDARD TABLE OF zzz_r_agency_reviewtp,
      reviews_to_test    TYPE TABLE FOR INSTANCE FEATURES KEY zi_agencytp\\zzz_review,
      requested_features TYPE STRUCTURE FOR INSTANCE FEATURES REQUEST zi_agencytp\\zzz_review,
      act_result         TYPE TABLE FOR INSTANCE FEATURES RESULT zi_agencytp\\zzz_review,
      exp_result         TYPE TABLE FOR INSTANCE FEATURES RESULT zi_agencytp\\zzz_review,
      reported           TYPE RESPONSE FOR REPORTED EARLY zi_agencytp,
      failed             TYPE RESPONSE FOR FAILED   EARLY zi_agencytp.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST zi_agencytp\\zzz_review,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT zi_agencytp\\zzz_review,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY zi_agencytp.

    requested_authorizations-%action-Zreviewwashelpful    = if_abap_behv=>mk-on.
    requested_authorizations-%action-Zreviewwasnothelpful = if_abap_behv=>mk-on.
  ...
  METHOD ratinginrange_valid.
    DATA: review_mock_data TYPE STANDARD TABLE OF zzz_r_agency_reviewtp.
    ...
    DATA reported TYPE RESPONSE FOR REPORTED LATE zi_agencytp.
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        act = lines( reported-zzz_review )
      ).
  ENDMETHOD.

  METHOD ratinginrange_invalid.
    TYPES BEGIN OF ts_review_test_data.
    INCLUDE TYPE zzz_r_agency_reviewtp.
    ...
    DATA:
      ...
      review_mock_data          TYPE STANDARD TABLE OF zzz_r_agency_reviewtp,
      reported                  TYPE RESPONSE FOR REPORTED LATE  zi_agencytp,
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE zi_agencytp\\zzz_review,
      reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE zi_agencytp\\zzz_review.

    review_test_data = VALUE #(
        agencyid = '1'
        ( reviewid = '1'  rating = '0'  t100 = CORRESPONDING #( zzz_cx_agency_review=>rating_invalid ) )
        ( reviewid = '2'  rating = '7'  t100 = CORRESPONDING #( zzz_cx_agency_review=>rating_invalid ) )
      ).
    ...
    LOOP AT review_test_data ASSIGNING FIELD-SYMBOL(<reviewy>).
      ...
      LOOP AT reported-zzz_review INTO DATA(reported_line) USING KEY entity
        ...
        cl_abap_unit_assert=>assert_equals(
            exp = lhc_zzz_review=>rating_in_range
  ...
  METHOD _reviewhelpful.
    DATA: review_mock_data TYPE STANDARD TABLE OF zzz_r_agency_reviewtp,
          act_reviews      TYPE TABLE FOR READ RESULT zi_agencytp\\zzz_review,
          exp_reviews      TYPE TABLE FOR READ RESULT zi_agencytp\\zzz_review.
    ...
    DATA reported TYPE RESPONSE FOR REPORTED EARLY zi_agencytp.
...
CLASS ltcl_sc_r_agency IMPLEMENTATION.
  ...
  METHOD class_setup.
    CREATE OBJECT class_under_test FOR TESTING.
    sql_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZZZ_AGN_REVA' ) ) ).
  ENDMETHOD.
  ...
  METHOD adjust_numbers.
    DATA:
      act_mapped       TYPE RESPONSE FOR MAPPED LATE zi_agencytp,
      exp_mapped       TYPE RESPONSE FOR MAPPED LATE zi_agencytp,
      review_mock_data TYPE STANDARD TABLE OF zzz_agn_reva,
      reported         TYPE RESPONSE FOR REPORTED LATE zi_agencytp.
    ...
    act_mapped-zzz_review = CORRESPONDING #( exp_mapped-zzz_review MAPPING %pid = %pid  %tmp = %tmp EXCEPT * ).
