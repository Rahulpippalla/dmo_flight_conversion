...
CLASS ltcl_agency IMPLEMENTATION.
  ...
  METHOD validatecountry_valid.
    ...
    DATA agency_mock_data TYPE STANDARD TABLE OF zagency.
  ...
  METHOD validatecountry_invalidnumber.
    ...
    DATA:
      ...
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE zi_agencytp,
      reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE zi_agencytp.

    agency_test_data = VALUE #(
        ( agency_id = '1' phone_number = '49'     country_code = 'DE'  t100 = CORRESPONDING #( zzz_cx_agency_country=>number_invalid      ) )
        ( agency_id = '2' phone_number = '49'     country_code = 'D'   t100 = CORRESPONDING #( zzz_cx_agency_country=>number_invalid      ) )
        ( agency_id = '3' phone_number = '+49'    country_code = 'D'   t100 = CORRESPONDING #( zzz_cx_agency_country=>combination_invalid ) )
        ( agency_id = '4' phone_number = '008955' country_code = 'DE'  t100 = CORRESPONDING #( zzz_cx_agency_country=>combination_invalid ) )
      ).
    ...
    LOOP AT agency_test_data ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      LOOP AT reported-zagency INTO DATA(reported_line) USING KEY entity WHERE agencyid = <agency>-agency_id.
  ...
  METHOD determinecountrycode.
    TYPES BEGIN OF ts_agency_test_data.
    INCLUDE TYPE zagency.
    TYPES exp_country_code TYPE zagency-country_code.
    TYPES END OF ts_agency_test_data.
    TYPES tt_agency_test_data TYPE STANDARD TABLE OF ts_agency_test_data.
  ...
  METHOD determinediallingcode.
    TYPES BEGIN OF ts_agency_test_data.
    INCLUDE TYPE zagency.
    TYPES exp_phone_number TYPE zagency-phone_number.
    TYPES END OF ts_agency_test_data.
    TYPES tt_agency_test_data TYPE STANDARD TABLE OF ts_agency_test_data.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST zi_agencytp\\zagency,
  ...
  METHOD createfromtemplate_valid.
    ...
    DATA:
      agency_mock_data TYPE STANDARD TABLE OF zagency,
      mapped           TYPE RESPONSE FOR MAPPED EARLY zi_agencytp,
      reported         TYPE RESPONSE FOR REPORTED EARLY zi_agencytp,
      failed           TYPE RESPONSE FOR FAILED EARLY zi_agencytp.

    DATA(agency_to_test) = VALUE zagency(
        agency_id             = '1'
        name                  = 'Test'
        street                = 'Street'
        postal_code           = '123'
    ...
    cl_abap_unit_assert=>assert_initial( mapped-zzz_review ).

    cl_abap_unit_assert=>assert_not_initial( mapped-zagency ).
    cl_abap_unit_assert=>assert_equals(
        act = lines( mapped-zagency )
        exp = 1
      ).

    DATA(mapped_line) = mapped-zagency[ 1 ].
    cl_abap_unit_assert=>assert_equals(
        act = mapped_line-%cid
        exp = cid
      ).
    ...
    READ ENTITIES OF zi_agencytp
      ENTITY zagency
        FIELDS (
          ...
          filename
          zzzsloganzag
        ) WITH CORRESPONDING #( mapped-zagency )
  ...
  METHOD createfromtemplate_invalid.
    ...
    cl_abap_unit_assert=>assert_initial( failed-zzz_review ).

    cl_abap_unit_assert=>assert_equals(
        act = lines( failed-zagency )
        exp = 1
      ).

    cl_abap_unit_assert=>assert_equals(
        act = failed-zagency[ 1 ]-%fail-cause
