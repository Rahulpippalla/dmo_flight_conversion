...
CLASS ltc_agency_handler DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mapped        TYPE RESPONSE FOR MAPPED   EARLY ZR_AgencyTP,
      failed        TYPE RESPONSE FOR FAILED   EARLY ZR_AgencyTP,
      reported      TYPE RESPONSE FOR REPORTED EARLY ZR_AgencyTP,
      failed_late   TYPE RESPONSE FOR FAILED   LATE  ZR_AgencyTP,
      reported_late TYPE RESPONSE FOR REPORTED LATE  ZR_AgencyTP.
...
CLASS ltc_agency_handler IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    sql_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZR_AgencyTP' ) ( 'I_COUNTRY' ) ) ).
  ENDMETHOD.
  ...
  METHOD validate_name_success.
    DATA:
      agency   TYPE ZR_AgencyTP,
      agencies TYPE STANDARD TABLE OF ZR_AgencyTP.

    agency = VALUE ZR_AgencyTP(
        AgencyId = '123'
        Name     = 'FlightBooker'
      ).
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        exp = 1
        act = lines( reported_late-ZAgency )
      ).
    DATA(reported_agency) = reported_late-ZAgency[ 1 ].
  ...
  METHOD validate_name_failed.
    DATA:
      agency                    TYPE ZR_AgencyTP,
      agencies                  TYPE STANDARD TABLE OF ZR_AgencyTP,
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZR_AgencyTP,
      reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE ZR_AgencyTP.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 1  act = lines( failed_late-ZAgency ) ).

    LOOP AT failed-ZAgency INTO DATA(failed_line).
    ...
    LOOP AT reported_late-ZAgency INTO DATA(reported_line).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( zcx_agency=>name_required )
  ...
  METHOD validate_email_failed.
    CONSTANTS: cv_found TYPE ZR_AgencyTP-Name VALUE 'found' ##NO_TEXT.
    DATA:
      agency                    TYPE ZR_AgencyTP,
      agencies                  TYPE STANDARD TABLE OF ZR_AgencyTP,
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZR_AgencyTP,
    ...
    LOOP AT agencies ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      LOOP AT reported_late-ZAgency INTO DATA(reported_line) USING KEY entity WHERE AgencyId = <agency>-AgencyId.
        IF reported_line-%msg IS BOUND.
          reported_with_message     = reported_line.
        ELSE.
          reported_clear_state_area = reported_line.
      ...
      cl_abap_unit_assert=>assert_equals(
          exp = CORRESPONDING symsg( zcx_agency=>email_invalid_format )
          act = CORRESPONDING symsg( reported_with_message-%msg->if_t100_message~t100key )
        ).
    ENDLOOP.
...
*    cds_test_environment->insert_test_data( agencies ).
  ...
  METHOD validate_countrycode_failed.
    ...
    LOOP AT agencies ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      cl_abap_unit_assert=>assert_equals(
          exp = CORRESPONDING symsg( zcx_agency=>country_code_invalid )
          act = CORRESPONDING symsg( reported_with_message-%msg->if_t100_message~t100key )
        ).
    ENDLOOP.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZR_AgencyTP\\ZAgency,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZR_AgencyTP\\ZAgency,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY ZR_AgencyTP.
...
CLASS ltc_agency_saver DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mapped   TYPE RESPONSE FOR MAPPED   LATE ZR_AgencyTP,
      reported TYPE RESPONSE FOR REPORTED LATE ZR_AgencyTP.
...
CLASS ltc_agency_saver IMPLEMENTATION.
  ...
  METHOD latenumbering_idempotency.
    CONSTANTS: cv_agencyid TYPE ZR_AgencyTP-AgencyId VALUE '123'.

    DATA:
      act_mapped_line LIKE LINE OF mapped-ZAgency,
      exp_mapped_line LIKE LINE OF mapped-ZAgency.

    mapped-ZAgency = VALUE #( ( AgencyId = cv_agencyid ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = 1
        act = lines( mapped-ZAgency )
      ).

    cl_abap_unit_assert=>assert_equals(
        exp = cv_agencyid
        act = mapped-ZAgency[ 1 ]-AgencyId
      ).
  ENDMETHOD.
  ...
  METHOD latenumbering_new_number.
    ...
    mapped-ZAgency = VALUE #( ( %pid = cv_pid ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = cv_pid
        act = mapped-ZAgency[ 1 ]-%pid
      ).

    cl_abap_unit_assert=>assert_not_initial( mapped-ZAgency[ 1 ]-AgencyId ).
  ENDMETHOD.
