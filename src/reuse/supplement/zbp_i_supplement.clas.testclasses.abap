...
CLASS ltc_supplement DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mapped        TYPE RESPONSE FOR MAPPED   EARLY zi_supplement,
      failed        TYPE RESPONSE FOR FAILED   EARLY zi_supplement,
      reported      TYPE RESPONSE FOR REPORTED EARLY zi_supplement,
      failed_late   TYPE RESPONSE FOR FAILED   LATE  zi_supplement,
      reported_late TYPE RESPONSE FOR REPORTED LATE  zi_supplement.
...
CLASS ltc_supplement IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create( i_for_entity = 'ZI_SUPPLEMENT' ).
  ENDMETHOD.
  ...
  METHOD validate_price_success.
    DATA:
      supplement  TYPE zsupplement,
      supplements TYPE STANDARD TABLE OF zsupplement.

    supplement = VALUE zsupplement(
        supplement_id       = 'XX123'
  ...
  METHOD validate_price_failed.
    ...
    DATA:
      supplements     TYPE STANDARD TABLE OF zsupplement WITH KEY supplement_id,
  ...
  METHOD earlynumbering_idempotency.
    DATA:
      entity          TYPE STRUCTURE FOR CREATE zi_supplement\\supplement,
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST zi_supplement\\supplement,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT zi_supplement\\supplement,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY zi_supplement.
