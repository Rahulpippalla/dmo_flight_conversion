...
CLASS lcl_test_double DEFINITION FOR TESTING.
  PUBLIC SECTION.
    ...
    CONSTANTS:
      supplement_entity_name TYPE abp_root_entity_name VALUE 'ZI_SUPPLEMENT' ##NO_TEXT,
      text_entity_name       TYPE abp_root_entity_name VALUE 'ZI_SUPPLEMENTTEXT' ##NO_TEXT,
      text_sub_name          TYPE abp_behv_changes-sub_name VALUE '_SUPPLEMENTTEXT' ##NO_TEXT.

    DATA:
      supplement_create TYPE TABLE FOR CREATE ZI_Supplement\\Supplement,
      supplement_update TYPE TABLE FOR UPDATE ZI_Supplement\\Supplement,
      supplement_failed TYPE RESPONSE FOR FAILED ZI_Supplement,

      text_create       TYPE TABLE FOR CREATE ZI_Supplement\\Supplement\_SupplementText,
      text_update       TYPE TABLE FOR UPDATE ZI_Supplement\\SupplementText,

      text_rba_link     TYPE TABLE FOR READ LINK   ZI_Supplement\\Supplement\_SupplementText,

      text_read_failed  TYPE RESPONSE FOR FAILED ZI_Supplement.
ENDCLASS.
...
CLASS lcl_test_double IMPLEMENTATION.
  ...
  METHOD if_abap_behavior_testdouble~read.
    TYPES:
      key         TYPE STRUCTURE FOR READ IMPORT ZI_Supplement\\Supplement\_SupplementText.
...
CLASS ltcl_augmentation DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mapped   TYPE RESPONSE FOR MAPPED EARLY zc_supplement,
      failed   TYPE RESPONSE FOR FAILED EARLY zc_supplement,
      reported TYPE RESPONSE FOR REPORTED EARLY zc_supplement.
...
CLASS ltcl_augmentation IMPLEMENTATION.
  ...
  METHOD supplement_create.
    DATA:
      c_create TYPE STRUCTURE FOR CREATE ZC_Supplement\\Supplement,
      i_create TYPE STRUCTURE FOR CREATE ZI_Supplement\\Supplement.
    ...
    MODIFY ENTITIES OF zc_supplement
      ENTITY Supplement
  ...
  METHOD supplement_create_text_update.
    ...
    DATA:
      c_create TYPE STRUCTURE FOR CREATE ZC_Supplement\\Supplement,
      i_create TYPE STRUCTURE FOR CREATE ZI_Supplement\\Supplement,
      c_update TYPE STRUCTURE FOR UPDATE ZC_Supplement\\Supplement,
      i_update TYPE STRUCTURE FOR UPDATE ZI_Supplement\\Supplement.
  ...
  METHOD update_with_invalid_keys.
    ...
    DATA:
      c_update_no_key      TYPE STRUCTURE FOR UPDATE ZC_Supplement\\Supplement,
      c_update_invalid_key TYPE STRUCTURE FOR UPDATE ZC_Supplement\\Supplement,
      i_update             TYPE STRUCTURE FOR UPDATE ZI_Supplement\\Supplement.
  ...
  METHOD update_invalid_keys_descr.
    ...
    DATA:
      c_update_no_flag     TYPE STRUCTURE FOR UPDATE ZC_Supplement\\Supplement,
      c_update_invalid_key TYPE STRUCTURE FOR UPDATE ZC_Supplement\\Supplement,
      i_update             TYPE STRUCTURE FOR UPDATE ZI_Supplement\\Supplement.
