CLASS ltcl_mapping DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    METHODS:
    "! Checks via check table if the current implementation of {@link ZCL_TRAVEL_AUXILIARY.meth:GET_CAUSE_FROM_MESSAGE }.
  ...
  METHOD all_in_one.
    ...
    LOOP AT exp INTO DATA(test_case).
      APPEND VALUE check_line(
          ...
          fail_cause   = zcl_travel_auxiliary=>get_cause_from_message(
                           msgid        = test_case-msgid
                           msgno        = test_case-msgno
                           is_dependend = test_case-is_dependend
                         )
