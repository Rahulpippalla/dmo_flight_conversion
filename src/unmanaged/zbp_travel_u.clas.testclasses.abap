CLASS ltcl_handler DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CONSTANTS:
      fm_create      TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_CREATE',
      fm_delete      TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_DELETE',
      fm_read        TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_READ',
      fm_set_booking TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_SET_BOOKING',
    ...
    METHODS:
      "! Checks if { @link ..lhc_travel.METH:create } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_CREATE } succeeds.
      create_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:create } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_CREATE } fails.
      create_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:update } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } succeeds.
      update_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:update } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } fails.
      update_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:delete } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_DELETE } succeeds.
      delete_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:delete } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_DELETE } fails.
      delete_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:read } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds.
      read_success              FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:read } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } fails.
      read_fail                 FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:rba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds.
      rba_success               FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:rba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } fails.
      rba_fail                  FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } and { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } both succeeds.
      cba_success               FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } fails.
      cba_fail_read             FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds but { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } fails.
      cba_fail_update           FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:set_status_booked } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_SET_BOOKING } succeeds.
      set_status_booked_success FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:set_status_booked } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_SET_BOOKING } fails.
...
CLASS ltcl_handler IMPLEMENTATION.
  ...
  METHOD create_success.
    DATA:
      entities    TYPE TABLE FOR CREATE zi_travel_u\\travel.


    DATA(fm_create_double) = fm_test_environment->get_double( fm_create ).

    DATA(travel) = VALUE ztravel( travel_id = travel_id ).
  ...
  METHOD update_success.
    DATA:
      entities    TYPE TABLE FOR UPDATE zi_travel_u\\travel.
  ...
  METHOD delete_success.
    DATA:
      keys    TYPE TABLE FOR DELETE zi_travel_u\\travel.
  ...
  METHOD read_success.
    DATA:
      keys        TYPE TABLE FOR READ IMPORT zi_travel_u\\travel,
      result      TYPE TABLE FOR READ RESULT zi_travel_u\\travel,
      result_line TYPE STRUCTURE FOR READ RESULT zi_travel_u\\travel.
  ...
  METHOD rba_success.
    DATA:
      keys_rba               TYPE TABLE     FOR READ IMPORT zi_travel_u\\travel\_Booking,
      result                 TYPE TABLE     FOR READ RESULT zi_travel_u\\travel\_Booking,
      result_line            TYPE STRUCTURE FOR READ RESULT zi_travel_u\\travel\_Booking,
      association_links      TYPE TABLE     FOR READ LINK   zi_travel_u\\travel\_Booking,
      association_links_line TYPE STRUCTURE FOR READ LINK   zi_travel_u\\travel\_Booking,
  ...
  METHOD get_instance_feature.
    TYPES:
      BEGIN OF check_line,
        travel_id                     TYPE ztravel-travel_id,
        status                        TYPE ztravel-status,
    ...
    DATA:
      keys               TYPE TABLE     FOR INSTANCE FEATURES KEY     zi_travel_u\\travel,
      requested_features TYPE STRUCTURE FOR INSTANCE FEATURES REQUEST zi_travel_u\\travel,
      result             TYPE TABLE     FOR INSTANCE FEATURES RESULT  zi_travel_u\\travel,
      result_line        TYPE STRUCTURE FOR INSTANCE FEATURES RESULT  zi_travel_u\\travel,
      import_read        TYPE STRUCTURE FOR READ              IMPORT  zi_travel_u\\travel,
      import_read_tab    TYPE TABLE     FOR READ              IMPORT  zi_travel_u\\travel,
      result_read        TYPE TABLE     FOR READ              RESULT  zi_travel_u\\travel.
...
*                           ).
*    DATA(env_config) = cl_abap_behv_test_environment=>create_environment_config(  )->set_bdef_dependencies( bdef_dependencies = VALUE #( ( 'ZI_TRAVEL_U' ) ) ).
*    DATA(bo_double_environment) = cl_abap_behv_test_environment=>create( environment_config = env_config ).
*
*    DATA(bo_double) = bo_double_environment->get_test_double( 'ZI_TRAVEL_U' ).
*
...
*      data(bo_double_input) = bo_double->create_read_input_for_entity( 'ZI_TRAVEL_U'   "can accept the entity name
...
*    ENDLOOP.
  ...
  METHOD set_status_booked_success.
    DATA:
      result      TYPE TABLE     FOR ACTION RESULT zi_travel_u\\travel~set_status_booked,
      result_line TYPE STRUCTURE FOR ACTION RESULT zi_travel_u\\travel~set_status_booked.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST zi_travel_u\\travel,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT zi_travel_u\\travel.
...
CLASS ltcl_saver DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CONSTANTS:
      fm_initialize     TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_INITIALIZE',
      fm_adjust_numbers TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_ADJ_NUMBERS',
      fm_save           TYPE sxco_fm_name VALUE 'ZFLIGHT_TRAVEL_SAVE'.
    ...
    DATA:
      ...
      mapped       TYPE RESPONSE FOR MAPPED   LATE zi_travel_u,
      failed       TYPE RESPONSE FOR FAILED   LATE zi_travel_u,
      reported     TYPE RESPONSE FOR REPORTED LATE zi_travel_u.
    ...
    METHODS:
      ...
      "! Checks if { @link ..lsc_i_travel_u.METH:adjust_numbers } mapping from { @link FUNC:ZFLIGHT_TRAVEL_ADJ_NUMBERS } works as expected.
      adjust_numbers    FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lsc_i_travel_u.METH:save } calls { @link FUNC:ZFLIGHT_TRAVEL_SAVE } and does not raise any exception.
      save              FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lsc_i_travel_u.METH:cleanup } calls { @link FUNC:ZFLIGHT_TRAVEL_INITIALIZE } and does not raise any exception.
...
*        mapped   = mapped
  ...
  METHOD adjust_numbers.
    CONSTANTS:
      travel_pre_1               TYPE ztravel_id VALUE '9990',
      travel_pre_2               TYPE ztravel_id VALUE '9995',
      travel_final_1             TYPE ztravel_id VALUE '6310',
      travel_final_2             TYPE ztravel_id VALUE '6315',
      booking_pre_1              TYPE zbooking_id VALUE '990',
      booking_pre_2              TYPE zbooking_id VALUE '995',
      booking_final_1            TYPE zbooking_id VALUE '410',
      booking_final_2            TYPE zbooking_id VALUE '415',
      booking_supplement_pre_1   TYPE zbooking_supplement_id VALUE '90',
      booking_supplement_pre_2   TYPE zbooking_supplement_id VALUE '95',
      booking_supplement_final_1 TYPE zbooking_supplement_id VALUE '10',
      booking_supplement_final_2 TYPE zbooking_supplement_id VALUE '15'.


    DATA:
      travel_mapping       TYPE zif_flight_legacy=>tt_ln_travel_mapping,
      booking_mapping      TYPE zif_flight_legacy=>tt_ln_booking_mapping,
      bookingsuppl_mapping TYPE zif_flight_legacy=>tt_ln_bookingsuppl_mapping.

    travel_mapping = VALUE zif_flight_legacy=>tt_ln_travel_mapping(
        ( preliminary = VALUE zif_flight_legacy=>ts_ln_travel( travel_id = travel_pre_1   )
          final       = VALUE zif_flight_legacy=>ts_ln_travel( travel_id = travel_final_1 ) )
        ( preliminary = VALUE zif_flight_legacy=>ts_ln_travel( travel_id = travel_pre_2   )
          final       = VALUE zif_flight_legacy=>ts_ln_travel( travel_id = travel_final_2 ) )
      ).

    booking_mapping = VALUE zif_flight_legacy=>tt_ln_booking_mapping(
        ( preliminary = VALUE zif_flight_legacy=>ts_ln_booking( travel_id = travel_pre_1    booking_id = booking_pre_1   )
          final       = VALUE zif_flight_legacy=>ts_ln_booking( travel_id = travel_final_1  booking_id = booking_final_1 ) )
        ( preliminary = VALUE zif_flight_legacy=>ts_ln_booking( travel_id = travel_pre_1    booking_id = booking_pre_2   )
          final       = VALUE zif_flight_legacy=>ts_ln_booking( travel_id = travel_final_1  booking_id = booking_final_2 ) )
      ).

    bookingsuppl_mapping = VALUE zif_flight_legacy=>tt_ln_bookingsuppl_mapping(
        ( preliminary = VALUE zif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_pre_1    booking_id = booking_pre_1    booking_supplement_id = booking_supplement_pre_1   )
          final       = VALUE zif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_final_1  booking_id = booking_final_1  booking_supplement_id = booking_supplement_final_1 ) )
        ( preliminary = VALUE zif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_pre_1    booking_id = booking_pre_1    booking_supplement_id = booking_supplement_pre_2   )
          final       = VALUE zif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_final_1  booking_id = booking_final_1  booking_supplement_id = booking_supplement_final_2 ) )
      ).
