CLASS ltcl_handler DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      "! Checks if { @link ..lhc_bookingsupplement.METH:read } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds and the <em>Booking_ID</em> is found.
      read_success                FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_bookingsupplement.METH:read } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds but the <em>Booking_Supplement_ID</em> to be read is not found.
      read_wrong_booking_suppl_id FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_bookingsupplement.METH:read } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } fails.
      read_fail                   FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_bookingsupplement.METH:update } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } succeeds.
      update_success              FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_bookingsupplement.METH:update } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } fails.
      update_fail                 FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_bookingsupplement.METH:delete } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } succeeds.
      delete_success              FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_bookingsupplement.METH:delete } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_UPDATE } fails.
      delete_fail                 FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_bookingsupplement.METH:rba_Travel } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds.
      rba_travel_success          FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_bookingsupplement.METH:rba_Travel } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } fails.
      rba_travel_fail             FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_bookingsupplement.METH:rba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } succeeds.
      rba_booking_success         FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_bookingsupplement.METH:rba_Booking } works as expected when { @link FUNC:ZFLIGHT_TRAVEL_READ } fails.
...
CLASS ltcl_handler IMPLEMENTATION.
  ...
  METHOD read_success.
    DATA:
      result              TYPE TABLE     FOR READ RESULT zi_travel_u\\BookingSupplement,
      result_line         TYPE STRUCTURE FOR READ RESULT zi_travel_u\\BookingSupplement,
  ...
  METHOD update_success.
    DATA:
      entities    TYPE TABLE FOR UPDATE zi_travel_u\\BookingSupplement.
    ...
    DATA(booking_supplementsx) = VALUE zt_booking_supplement_inx( (
  ...
  METHOD delete_success.
    ...
    DATA(booking_supplementsx) = VALUE zt_booking_supplement_inx( (            booking_id = booking_id  booking_supplement_id = booking_supplement_id  action_code = zif_flight_legacy=>action_code-delete ) ).
  ...
  METHOD rba_travel_success.
    DATA:
      keys                   TYPE TABLE     FOR READ IMPORT zi_travel_u\\BookingSupplement\_Travel,
      result                 TYPE TABLE     FOR READ RESULT zi_travel_u\\BookingSupplement\_Travel,
      result_line            TYPE STRUCTURE FOR READ RESULT zi_travel_u\\BookingSupplement\_Travel,
      association_links      TYPE TABLE     FOR READ LINK   zi_travel_u\\BookingSupplement\_Travel,
      association_links_line TYPE STRUCTURE FOR READ LINK   zi_travel_u\\BookingSupplement\_Travel.
  ...
  METHOD rba_booking_success.
    DATA:
      result                 TYPE TABLE     FOR READ RESULT zi_travel_u\\BookingSupplement\_Booking,
      result_line            TYPE STRUCTURE FOR READ RESULT zi_travel_u\\BookingSupplement\_Booking,
      association_links      TYPE TABLE     FOR READ LINK   zi_travel_u\\BookingSupplement\_Booking,
      association_links_line TYPE STRUCTURE FOR READ LINK   zi_travel_u\\BookingSupplement\_Booking,
