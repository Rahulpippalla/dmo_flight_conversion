CLASS ztc_flight_travel_api DEFINITION
  PUBLIC
  ABSTRACT
  CREATE PUBLIC
  FOR TESTING
...
CLASS ztc_flight_travel_api IMPLEMENTATION.
  ...
  METHOD cuerd_travel_early_numbering.
    ...
    CALL FUNCTION 'ZFLIGHT_TRAVEL_SAVE'.

    " DB Check
    SELECT SINGLE agency_id, customer_id, description FROM ztravel WHERE travel_id = @lv_travel_id INTO ( @DATA(lv_agency_id), @DATA(lv_customer_id), @DATA(lv_description) ).
    ...
    " Action
    CALL FUNCTION 'ZFLIGHT_TRAVEL_SET_BOOKING'
      EXPORTING
    ...
    " Read DB only
    CLEAR ls_travel.
    CALL FUNCTION 'ZFLIGHT_TRAVEL_READ'
      EXPORTING
        iv_travel_id      = lv_travel_id
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel-status       exp = CONV ztravel_status( zif_flight_legacy=>travel_status-new ) ).
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel-status       exp = CONV ztravel_status( zif_flight_legacy=>travel_status-booked ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel-description  exp = 'My New Test' ).

    " Delete
    CALL FUNCTION 'ZFLIGHT_TRAVEL_DELETE'
      EXPORTING
        iv_travel_id = lv_travel_id
      IMPORTING
        et_messages  = lt_messages.
    ...
    " Rollback
    CALL FUNCTION 'ZFLIGHT_TRAVEL_INITIALIZE'.
  ENDMETHOD.
  ...
  METHOD cuerd_travel_late_numbering.
    ...
    CALL FUNCTION 'ZFLIGHT_TRAVEL_ADJ_NUMBERS'
      IMPORTING
