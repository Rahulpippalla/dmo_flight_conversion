...
FUNCTION zflight_travel_calc_price.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  INCLUDE TYPE zflight.
  TYPES   distance       TYPE zconnection-distance.
  TYPES   distance_unit  TYPE zconnection-distance_unit.
  TYPES END OF ty_seats_occupied.

  TYPES: tt_flight TYPE STANDARD TABLE OF zflight WITH KEY client carrier_id connection_id flight_date.

  DATA: lt_seats_occupied          TYPE TABLE OF ty_seats_occupied.

  IF it_flight IS SUPPLIED.
    SELECT FROM zbooking AS booking
      JOIN     zconnection AS connection
                 ON  booking~carrier_id    = connection~carrier_id
                 AND booking~connection_id = connection~connection_id
      JOIN     zflight AS flight
