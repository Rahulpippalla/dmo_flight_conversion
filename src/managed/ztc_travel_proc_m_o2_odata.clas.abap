"!@testing SRVB:ZUI_TRAVEL_PROC_M_O2
...
CLASS ztc_travel_proc_m_o2_odata DEFINITION
  FOR TESTING
  RISK LEVEL HARMLESS
  DURATION SHORT
  PUBLIC
  ...
  PRIVATE SECTION.
    CLASS-DATA: mo_client_proxy      TYPE REF TO /iwbep/if_cp_client_proxy,
                ...
                begin_date           TYPE zbegin_date,
                end_date             TYPE zend_date,
                agency_mock_data     TYPE STANDARD TABLE OF zagency,
                customer_mock_data   TYPE STANDARD TABLE OF zcustomer,
                carrier_mock_data    TYPE STANDARD TABLE OF zcarrier,
...
CLASS ztc_travel_proc_m_o2_odata  IMPLEMENTATION.

  METHOD class_setup.

    mo_client_proxy = create_local_client_proxy( VALUE #( service_id      = 'ZUI_TRAVEL_PROC_M_O2'
                                                          service_version = '0001' )  ).

    " Create the stubs/doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZC_TRAVEL_PROCESSOR_M'    i_select_base_dependencies = abap_true )
                        ( i_for_entity = 'ZC_BOOKING_PROCESSOR_M'   i_select_base_dependencies = abap_true )
                        ( i_for_entity = 'ZC_BOOKSUPPL_PROCESSOR_M' i_select_base_dependencies = abap_true ) ) ).
...
*        ( 'ZAGENCY' )      "already mocked by cds_test_environment above
*        ( 'ZCUSTOMER' )    "already mocked by cds_test_environment above
*        ( 'ZCARRIER' )     "already mocked by cds_test_environment above
        ( 'ZFLIGHT' )
        ( 'ZSUPPLEMENT' )
  ...
  METHOD create_travel.
    ...
    DATA(ls_business_data) = VALUE zc_travel_processor_m(
    ...
    DATA ls_response_data TYPE zc_travel_processor_m.
    ...
    READ ENTITIES OF zc_travel_processor_m
    ...
    SELECT single @abap_true FROM zi_travel_m INTO @DATA(lv_travel). "#EC CI_NOWHERE
    cl_abap_unit_assert=>assert_true( msg = 'travel from db' act = lv_travel ).

    " check also log written by "additional save" functionality
    SELECT single @abap_true FROM zlog_travel INTO @DATA(log_travel). "#EC CI_NOWHERE
    cl_abap_unit_assert=>assert_true( msg = 'ZLOG_TRAVEL' act = log_travel ).
