...
"! <li>The dependent BO is zi_travel_m and the code under test is its consumer {@link ztc_travel_m_bo_consumer}</li>
...
class ltcl_mockemlapi_variant_demos definition final for testing
  ...
  private section.
    ...
    class-data cut type ref to ztc_travel_m_bo_consumer.
...
class ltcl_mockemlapi_variant_demos implementation.
  ...
  method class_setup.
  "Create doubles for BO 'ZI_TRAVEL_M'.
  "a. Prepare environment configuration with bdef dependencies for which doubles are to be created
  data(env_config) = cl_botd_mockemlapi_bo_test_env=>prepare_environment_config(  )->set_bdef_dependencies( bdef_dependencies = value #( ( 'ZI_TRAVEL_M' ) ) ).
  ...
  "For Create on Travel entity
   data create_travel_instances type table for create ZI_TRAVEL_M.
   create_travel_instances = value #( ( %cid = 'Travel_987' Agency_ID = '000111' Customer_ID = '000006' description = 'Travel_987' %control-Agency_ID = if_abap_behv=>mk-on
                                      ...
                                      ( %cid = 'Travel_988' Agency_ID = '000111' Customer_ID = '000006' description = 'Travel_988' Begin_Date = sy-datum + 10 End_Date = sy-datum ) ).
  ...
  "Step 3: Create input and output configurations for the Modify EML.
    ...
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_modify->build_entity_part( 'zi_travel_m'
  ...
  "Step 4: Configure the Modify EML via configure_call API on the double.
    double =  environment->get_test_double( 'ZI_TRAVEL_M' ).
  ...
  "Step 1: Setup test data instances for all operations on all entities in Modify EML (Here only create_ba ).
   data cba_booking_instances type table for create ZI_TRAVEL_M\_Booking.
  ...
  "Step 3: Create input and output configurations for the Modify EML.
    ...
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_modify->build_entity_part( 'ZI_TRAVEL_M'   "can accept the entity name or alias name
  ...
  "For Update on Booking entity
   data update_booking_instances type table for update zi_travel_m\\booking.
  ...
  "Step 3: Create input and output configurations for the Modify EML.
    ...
    "Create input for all entity parts
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_modify->build_entity_part( 'zi_travel_m'   "can accept the entity name
                                   )->set_instances_for_create( create_travel_instances
                                   )->set_instances_for_create_ba( cba_booking_instances ).
  ...
  "*************Configure second EML in Code under test*************
    ...
    data create_travel_instances2 type table for create zi_travel_m.
    create_travel_instances2 = value #( ( %cid = 'Travel_987' Customer_ID = '000006' description = 'Travel_987' ) ).

    "travel input part for 2nd EML
    data(eml_travel_input2) = input_config_builder_4_modify->build_entity_part( 'zi_travel_m'   "can accept the entity name
  ...
  data(verification_input) = input_config_builder_4_modify->build_input_for_eml(
                                                                     ...
                                                                     input_config_builder_4_modify->build_entity_part( 'zi_travel_m'
                                                                            )->set_instances_for_create( create_travel_instances2 ) ).
  ...
  "Step 3: Create input and output configurations for the Modify EML.
   ...
   data(output_config_builder_4_modify) = cl_botd_mockemlapi_bldrfactory=>get_output_config_builder( )->for_modify( ).
    ...
    "Create input for all entity parts and set instances for all operations on that entity
    "For travel entity
   data(travel_part) = input_config_builder_4_modify->build_entity_part( 'zi_travel_m'   "can accept the entity name
                                   )->set_instances_for_create( create_travel_instances ).
  ...
  "For Update on Travel entity
   data update_travel_instances type table for update ZI_TRAVEL_M.
  ...
  "For accept travel action on Travel entity
   data accept_travel_act_insts type table for action import ZI_TRAVEL_M~acceptTravel.
   accept_travel_act_insts = value #( ( Travel_ID = '987'  ) "Should pass
                                      ( Travel_ID = '988'  ) ).
                                                                                "Should fail, assuming Travel 988 does not exist

  "Step 2: Define and set up the response structures to be returned for Modify EML in CUT
    data mapped_accept_travel type response for mapped early ZI_TRAVEL_M.
    data failed type response for failed early ZI_TRAVEL_M.
    data result_accept_travel type table for action result ZI_TRAVEL_M~acceptTravel.
  ...
  "For Creating Booking by Association on Travel entity
   ...
   cba_booking_instances = value #( ( %cid_ref = 'Travel_987'
                                      %target = value #( (  %cid = 'Travel_987_Booking_001' %data-carrier_id = '000111' connection_id = 'C1' Customer_ID = '000006'
                                                            Flight_Date = sy-datum + 10 booking_date = sy-datum booking_status = 'B'  ) )  )  ).

  "For Creating booking_supplement by association on Booking entity
   data cba_booking_suppl_instances type table for create zi_travel_m\\booking\_BookSupplement.
  ...
  "For delete on Travel entity
   data delete_travel_instances type table for delete ZI_TRAVEL_M.
  ...
  "For read on travel
   data read_travel_instances type table for read import ZI_TRAVEL_M.
  ...
  "Step 2: Define and set up the response structures to be returned for Read EML in CUT
    data result type table for read result ZI_TRAVEL_M.
  ...
  "Step 3: Create input and output configurations for Read EML.
   ...
   data(output_config_builder_4_read) = cl_botd_mockemlapi_bldrfactory=>get_output_config_builder( )->for_read( ).
    ...
    "Create input for all entity parts
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_read->build_entity_part( 'ZI_TRAVEL_M'   "can accept the entity name
  ...
  "Step 3: Create input and output configurations for Read EML.
   ...
   data(output_config_builder_4_read) = cl_botd_mockemlapi_bldrfactory=>get_output_config_builder( )->for_read( ).
    ...
    "Create input for all entity parts
    "For travel entity
    data(eml_travel_input) = input_config_builder_4_read->build_entity_part( 'ZI_TRAVEL_M'   "can accept the entity name/alias name
                                   )->set_instances_for_read( read_travel_instances ).
  ...
  "For read booking by association on travel
   data rba_booking_instances type table for read import ZI_TRAVEL_M\_Booking.
  ...
  "Step 2: Define and set up the response structures to be returned for Read EML in CUT
    data result type table for read result ZI_TRAVEL_M\_Booking.
...
class ltcl_txbufdbl_variant_demos implementation.
  ...
  "Create doubles for BO 'ZI_TRAVEL_M'.
  "a. Prepare environment configuration with bdef dependencies for which doubles are to be created
    data(env_config) = cl_botd_txbufdbl_bo_test_env=>prepare_environment_config(  )->set_bdef_dependencies( bdef_dependencies = value #( ( 'ZI_TRAVEL_M' ) ) ).
  ...
  "Since early numbering is enabled for BO and the key fields are read-only, setting a numbering support is required to assign keys to instances for create or create_ba operations
    data(double) =  environment->get_test_double( 'ZI_TRAVEL_M' ).
...
*   data travel_instances type table for create zi_travel_m.
...
*    data(double) = environment->get_test_double( root_name = 'zi_travel_m' ).
...
*
  ...
  "Step 3: Define and set up the mapped response structure to be returned for Create EML on test data instance in CUT
    data mapped_987 type response for mapped early zi_travel_m.
    data mapped_988 type response for mapped early zi_travel_m.
  ...
  """""""""""""""""""""""""""CODE UNDER TEST"""""""""""""""""""""""""""""""""""""""
  "For Create on Travel entity
   data create_travel_instances_cut type table for create zi_travel_m.
  ...
  "Step 1: Insert test data in transactional buffer double using a CREATE EML (if required).
   ...
   data mapped_travel_987 type response for mapped early zi_travel_m.
  ...
  "Step 3: Define and set up the mapped response structure to be returned for Create by association EML on test data instance in CUT, with the key to be assigned for the instance.
    data mapped_booking_001 type response for mapped early zi_travel_m.
  ...
  "Step 3: Define and set up the mapped response structure to be returned for update EML on test data instance in CUT.
    data mapped_update type response for mapped early zi_travel_m.
