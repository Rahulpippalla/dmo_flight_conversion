...
PUBLIC SECTION.
  ...
  METHODS get_agency_data
            RETURNING VALUE(rs_agency) TYPE zagency.
  METHODS get_customer_data
            RETURNING VALUE(rs_customer) TYPE zcustomer.
  METHODS get_carrier_data
            RETURNING VALUE(rs_carrier) TYPE zcarrier.

PRIVATE SECTION.
   class-data gt_agency TYPE STANDARD TABLE OF zagency.
   class-data gt_customer TYPE STANDARD TABLE OF zcustomer.
   class-data gt_carrier TYPE STANDARD TABLE OF zcarrier.
