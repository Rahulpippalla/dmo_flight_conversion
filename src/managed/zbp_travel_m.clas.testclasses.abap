...
CLASS ltc_managed IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create( i_for_entity = 'ZI_TRAVEL_M' ).
    sql_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZCUSTOMER' ) ) ).
  ENDMETHOD.
  ...
  METHOD validate_travel_status_open.
    ...
    " Fill in test data
    DATA travel_mock_data TYPE STANDARD TABLE OF ztravel_m.
    ...
    DATA failed TYPE RESPONSE FOR FAILED LATE zi_travel_m.
    DATA reported TYPE RESPONSE FOR REPORTED LATE  zi_travel_m.
  ...
  METHOD get_features_open.
    ...
    DATA result  TYPE TABLE FOR INSTANCE FEATURES RESULT zi_travel_m\\travel.
    DATA failed TYPE RESPONSE FOR FAILED EARLY zi_travel_m.
    DATA reported TYPE RESPONSE FOR REPORTED EARLY  zi_travel_m.
  ...
  METHOD validate_customer_valid.
    ...
    DATA customer_mock_data TYPE STANDARD TABLE OF zcustomer.
  ...
  METHOD set_status_accepted.
    ...
    " Declare required table and structures
    DATA result   TYPE TABLE FOR ACTION RESULT zi_travel_m\\travel~accepttravel.
    DATA mapped   TYPE RESPONSE FOR MAPPED EARLY zi_travel_m.
    DATA failed   TYPE RESPONSE FOR FAILED EARLY zi_travel_m.
    DATA reported TYPE RESPONSE FOR REPORTED EARLY  zi_travel_m.
...
"! This class mocks the entity ZI_TRAVEL_M.
...
CLASS test_double_for_entity DEFINITION FOR TESTING.
  PUBLIC SECTION.
    ...
    DATA written TYPE TABLE FOR UPDATE zi_travel_m.
ENDCLASS.
...
CLASS test_double_for_entity IMPLEMENTATION.
  METHOD if_abap_behavior_testdouble~modify.
    LOOP AT changes ASSIGNING FIELD-SYMBOL(<change>).
      cl_abap_unit_assert=>assert_equals( msg = 'modify-mock entity' exp = 'ZI_TRAVEL_M' act = <change>-entity_name ).
  ...
  METHOD if_abap_behavior_testdouble~read.
    LOOP AT retrievals ASSIGNING FIELD-SYMBOL(<retrieval>).
      cl_abap_unit_assert=>assert_equals( msg = 'read-mock entity' exp = 'ZI_TRAVEL_M' act = <retrieval>-entity_name ).
...
CLASS test_using_entity_stub IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    entity_mock->if_abap_behavior_testdouble~root_name = 'ZI_TRAVEL_M'.
  ENDMETHOD.
  ...
  METHOD teardown.
    ...
    cl_abap_behv_test_environment=>unset_test_double( root = 'ZTRAVEL_D' ).
...
CLASS ltc_travl_not_in_documentation DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      travel_id1 TYPE ztravel_id VALUE '1',
      travel_id2 TYPE ztravel_id VALUE '2',
      travel_id3 TYPE ztravel_id VALUE '3',
      travel_id4 TYPE ztravel_id VALUE '4'.
...
CLASS ltc_travl_not_in_documentation IMPLEMENTATION.
  ...
  METHOD accepttravel.
    DATA:
      ...
      travels_to_test    TYPE STANDARD TABLE OF zi_travel_m WITH KEY travel_id,
      exp_travels_action TYPE TABLE FOR ACTION RESULT zi_travel_m\\travel~accepttravel,
      exp_travel_read    TYPE STRUCTURE FOR READ RESULT zi_travel_m\\travel,
      result             TYPE TABLE FOR ACTION RESULT  zi_travel_m\\travel~accepttravel,
  ...
  METHOD rejecttravel.
    DATA:
      travel_mock_data   TYPE STANDARD TABLE OF ztravel_m,
      travels_to_test    TYPE STANDARD TABLE OF zi_travel_m WITH KEY travel_id,
      exp_travels_action TYPE TABLE FOR ACTION RESULT zi_travel_m\\travel~rejecttravel,
      exp_travel_read    TYPE STRUCTURE FOR READ RESULT zi_travel_m\\travel,
      result             TYPE TABLE FOR ACTION RESULT  zi_travel_m\\travel~rejecttravel,
      mapped             TYPE RESPONSE FOR MAPPED EARLY  zi_travel_m,
      failed             TYPE RESPONSE FOR FAILED EARLY  zi_travel_m,
      reported           TYPE RESPONSE FOR REPORTED EARLY  zi_travel_m.
  ...
  METHOD recalctotalprice_one_currency.
    CONSTANTS:
      ...
      c_booking_fee      TYPE zbooking_fee VALUE '100',
      c_flight_price     TYPE zflight_price VALUE '10',
      c_supplement_price TYPE zsupplement_price VALUE '1'.

    CONSTANTS:
      booking_id1           TYPE zbooking_m-booking_id VALUE '10',
      booking_id2           TYPE zbooking_m-booking_id VALUE '20',
      bookingsupplement_id1 TYPE zbooksuppl_m-booking_supplement_id VALUE '1',
      bookingsupplement_id2 TYPE zbooksuppl_m-booking_supplement_id VALUE '2',
      bookingsupplement_id3 TYPE zbooksuppl_m-booking_supplement_id VALUE '3',
      bookingsupplement_id4 TYPE zbooksuppl_m-booking_supplement_id VALUE '4'.

    DATA:
      travel_mock_data            TYPE STANDARD TABLE OF ztravel_m,
      booking_mock_data           TYPE STANDARD TABLE OF zbooking_m,
      bookingsupplement_mock_data TYPE STANDARD TABLE OF zbooksuppl_m,
      travels_to_test             TYPE TABLE FOR ACTION IMPORT zi_travel_m\\travel~recalctotalprice,
      exp_travel_read             TYPE TABLE FOR READ RESULT zi_travel_m\\travel,
      mapped                      TYPE RESPONSE FOR MAPPED EARLY  zi_travel_m,
      failed                      TYPE RESPONSE FOR FAILED EARLY  zi_travel_m,
      reported                    TYPE RESPONSE FOR REPORTED EARLY  zi_travel_m.
  ...
  METHOD recalctotalprice_mix_currency.
    CONSTANTS:
      c_currency_eur     TYPE zcurrency_code VALUE 'EUR',
      c_currency_usd     TYPE zcurrency_code VALUE 'USD',
      c_booking_fee      TYPE zbooking_fee VALUE '100',
      c_flight_price     TYPE zflight_price VALUE '10',
      c_supplement_price TYPE zsupplement_price VALUE '1'.
  ...
  METHOD validatecustomer_success.
    CONSTANTS:
      c_customer_id TYPE zcustomer_id VALUE '123'.

    DATA:
      customer_mock_data TYPE STANDARD TABLE OF zcustomer,
      travel_mock_data   TYPE STANDARD TABLE OF ztravel_m,
      travels_to_test    TYPE TABLE FOR VALIDATION zi_travel_m\\travel~validatecustomer,
  ...
  METHOD validatecustomer_initial.
    CONSTANTS:
      c_customer_id           TYPE zcustomer_id VALUE '123',
      c_customer_id_of_travel TYPE zcustomer_id VALUE '111'.
  ...
  METHOD validatecustomer_not_exist.
    CONSTANTS:
      c_customer_id           TYPE zcustomer_id VALUE '123',
      c_customer_id_of_travel TYPE zcustomer_id VALUE IS INITIAL.
  ...
  METHOD validateagency_success.
    CONSTANTS:
      c_agency_id TYPE zagency_id VALUE '123'.

    DATA:
      agency_mock_data TYPE STANDARD TABLE OF zagency,
      travel_mock_data TYPE STANDARD TABLE OF ztravel_m,
      travels_to_test  TYPE TABLE FOR VALIDATION zi_travel_m\\travel~validateagency,
      failed           TYPE RESPONSE FOR FAILED LATE  zi_travel_m,
      reported         TYPE RESPONSE FOR REPORTED LATE  zi_travel_m.
  ...
  METHOD validateagency_initial.
    CONSTANTS:
      c_agency_id           TYPE zagency_id VALUE '123',
      c_agency_id_of_travel TYPE zagency_id VALUE '111'.
  ...
  METHOD validateagency_not_exist.
    CONSTANTS:
      c_agency_id           TYPE zagency_id VALUE '123',
      c_agency_id_of_travel TYPE zagency_id VALUE IS INITIAL.
  ...
  METHOD validatedates_success.
    DATA:
      travel_mock_data TYPE STANDARD TABLE OF ztravel_m,
      travels_to_test  TYPE TABLE FOR VALIDATION zi_travel_m\\travel~validatedates,
      failed           TYPE RESPONSE FOR FAILED LATE  zi_travel_m,
      reported         TYPE RESPONSE FOR REPORTED LATE  zi_travel_m,
  ...
  METHOD validatedates_not_valid.
    ...
    CONSTANTS:
      travel_id5 TYPE ztravel_id VALUE '5',
      travel_id6 TYPE ztravel_id VALUE '6'.
  ...
  METHOD get_instance_features.
    TYPES: t_instance_feature TYPE STRUCTURE FOR INSTANCE FEATURES RESULT zi_travel_m\\travel,
           BEGIN OF t_check.
             INCLUDE TYPE t_instance_feature.
    TYPES: overall_status TYPE zi_travel_m-overall_status,
           END OF t_check,
           t_check_table TYPE STANDARD TABLE OF t_check WITH KEY travel_id.
    ...
    DATA:
      ...
      travel_mock_data   TYPE STANDARD TABLE OF ztravel_m,
      travels_to_test    TYPE TABLE FOR INSTANCE FEATURES KEY zi_travel_m\\travel,
      requested_features TYPE STRUCTURE FOR INSTANCE FEATURES REQUEST zi_travel_m\\travel,
      act_result         TYPE TABLE FOR INSTANCE FEATURES RESULT zi_travel_m\\travel,
      exp_result         TYPE TABLE FOR INSTANCE FEATURES RESULT zi_travel_m\\travel,
      reported           TYPE RESPONSE FOR REPORTED EARLY zi_travel_m,
      failed             TYPE RESPONSE FOR FAILED   EARLY zi_travel_m.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST zi_travel_m\\travel,
      exp_result               TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT zi_travel_m\\travel,
      act_result               TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT zi_travel_m\\travel,
      reported                 TYPE RESPONSE FOR REPORTED EARLY zi_travel_m.
  ...
  METHOD earlynumbering_idempotency.
    DATA:
      entity          TYPE STRUCTURE FOR CREATE zi_travel_m\\travel,
      mapped          TYPE RESPONSE FOR MAPPED zi_travel_m,
      failed          TYPE RESPONSE FOR FAILED EARLY zi_travel_m,
      reported        TYPE RESPONSE FOR REPORTED EARLY zi_travel_m,
  ...
  METHOD copytravel.
    ...
    DATA:
      ...
      booking_mock_data           TYPE STANDARD TABLE OF zbooking_m,
      bookingsupplement_mock_data TYPE STANDARD TABLE OF zbooksuppl_m,
      travels_to_test             TYPE TABLE FOR ACTION IMPORT zi_travel_m\\travel~copytravel,
      exp_travel_read             TYPE TABLE FOR READ RESULT zi_travel_m\\travel,
      mapped                      TYPE RESPONSE FOR MAPPED EARLY  zi_travel_m,
      failed                      TYPE RESPONSE FOR FAILED EARLY  zi_travel_m,
      reported                    TYPE RESPONSE FOR REPORTED EARLY  zi_travel_m.
  ...
  METHOD validatestatus_success.
    DATA:
      travel_mock_data TYPE STANDARD TABLE OF ztravel_m,
      travels_to_test  TYPE TABLE FOR VALIDATION zi_travel_m\\travel~validatestatus,
      failed           TYPE RESPONSE FOR FAILED LATE  zi_travel_m,
      reported         TYPE RESPONSE FOR REPORTED LATE  zi_travel_m.
  ...
  METHOD earlynumbering_cba_new_number.
    DATA:
      ...
      failed            TYPE RESPONSE FOR FAILED EARLY  zi_travel_m,
      reported          TYPE RESPONSE FOR REPORTED EARLY  zi_travel_m,
      entities          TYPE TABLE FOR CREATE zi_travel_m\\travel\_booking.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = CONV zbooking_id( '10' )
        act = VALUE zbooking_id( mapped-booking[ %cid = '1b1' ]-booking_id )
      ).

    cl_abap_unit_assert=>assert_equals(
        exp = CONV zbooking_id( '20' )
        act = VALUE zbooking_id( mapped-booking[ %cid = '2b1' ]-booking_id )
      ).

    cl_abap_unit_assert=>assert_equals(
        exp = CONV zbooking_id( '20' )
        act = VALUE zbooking_id( mapped-booking[ %cid = '3b1' ]-booking_id )
      ).

    cl_abap_unit_assert=>assert_equals(
        exp = CONV zbooking_id( '40' )
        act = VALUE zbooking_id( mapped-booking[ %cid = '4b1' ]-booking_id )
      ).
  ENDMETHOD.
...
CLASS ltc_save_not_in_documentation IMPLEMENTATION.
  ...
  METHOD log_travels.
    TYPES:
      tt_logs TYPE STANDARD TABLE OF zlog_travel WITH KEY travel_id.

    DATA:
      create   TYPE REQUEST FOR CHANGE zi_travel_m,
      update   TYPE REQUEST FOR CHANGE zi_travel_m,
      delete   TYPE REQUEST FOR DELETE zi_travel_m,
  ...
  METHOD unmanaged_save_booksupplement.
    TYPES:
      tt_db_booking_supplement TYPE STANDARD TABLE OF zbooksuppl_m WITH KEY travel_id booking_id booking_supplement_id.
