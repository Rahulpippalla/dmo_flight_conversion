...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD validate_customer.
    ...
    DATA customers TYPE SORTED TABLE OF zcustomer WITH UNIQUE KEY customer_id.
      ...
      IF travel-customer_id IS INITIAL
        ...
        APPEND VALUE #(  %tky = travel-%tky
                         %msg      = NEW zcm_flight_messages(
                                         ...
                                         textid      = zcm_flight_messages=>customer_unkown
  ...
  METHOD validate_agency.
    ...
    DATA agencies TYPE SORTED TABLE OF zagency WITH UNIQUE KEY agency_id.
    ...
    IF  agencies IS NOT INITIAL.
      ...
      SELECT FROM zagency FIELDS agency_id
    ...
    LOOP AT travels INTO DATA(travel).
      IF travel-agency_id IS INITIAL
        ...
        APPEND VALUE #(  %tky = travel-%tky
                         %msg      = NEW zcm_flight_messages(
                                          textid    = zcm_flight_messages=>agency_unkown
  ...
  METHOD validate_dates.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-end_date < travel-begin_date.  "end_date before begin_date
        ...
        APPEND VALUE #( %tky = travel-%tky
                        %msg = NEW zcm_flight_messages(
                                   textid     = zcm_flight_messages=>begin_date_bef_end_date
                                   severity   = if_abap_behv_message=>severity-error
      ...
      ELSEIF travel-begin_date < cl_abap_context_info=>get_system_date( ).  "begin_date must be in the future
        ...
        APPEND VALUE #( %tky = travel-%tky
                        %msg = NEW zcm_flight_messages(
                                    textid   = zcm_flight_messages=>begin_date_on_or_bef_sysdate
                                    severity = if_abap_behv_message=>severity-error )
                        %element-begin_date  = if_abap_behv=>mk-on
                        %element-end_date    = if_abap_behv=>mk-on
                      ) TO reported-travel.
  ...
  METHOD copyTravel.
    ...
    DATA:
      travels       TYPE TABLE FOR CREATE ZI_Travel_M\\travel,
      bookings_cba  TYPE TABLE FOR CREATE ZI_Travel_M\\travel\_booking,
      booksuppl_cba TYPE TABLE FOR CREATE ZI_Travel_M\\booking\_booksupplement.
  ...
  METHOD earlynumbering_create.
    ...
    DATA:
      entity        TYPE STRUCTURE FOR CREATE ZI_Travel_M,
      travel_id_max TYPE ztravel_id.
      ...
      WHEN '1'.
        ...
        LOOP AT entities_wo_travelid INTO entity.
          APPEND VALUE #( %cid = entity-%cid
                          %key = entity-%key
                          %msg = NEW zcm_flight_messages(
                                      textid = zcm_flight_messages=>number_range_depleted
      ...
      WHEN '2' OR '3'.
        ...
        LOOP AT entities_wo_travelid INTO entity.
          APPEND VALUE #( %cid = entity-%cid
                          %key = entity-%key
                          %msg = NEW zcm_flight_messages(
                                      textid = zcm_flight_messages=>not_sufficient_numbers
                                      severity = if_abap_behv_message=>severity-warning )
                        ) TO reported-travel.
          APPEND VALUE #( %cid        = entity-%cid
                          %key        = entity-%key
  ...
  METHOD earlynumbering_cba_booking.
    DATA: max_booking_id TYPE zbooking_id.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<travel_group>) GROUP BY <travel_group>-travel_id.
      ...
      max_booking_id = REDUCE #( INIT max = CONV zbooking_id( '0' )
                                 FOR  booking IN bookings USING KEY entity WHERE ( source-travel_id  = <travel_group>-travel_id )
                                 NEXT max = COND zbooking_id( WHEN booking-target-booking_id > max
      ...
      max_booking_id = REDUCE #( INIT max = max_booking_id
                                 ...
                                 NEXT max = COND zbooking_id( WHEN   target-booking_id > max
...
********************************************************************************
    ...
    IF create-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log_c into the DB table zlog_travel
    ...
    IF update-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log_u into the DB table zlog_travel
    ...
    IF delete-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log into the DB table zlog_travel
